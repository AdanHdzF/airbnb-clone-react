...
type propertiesStore = {
	...
	// searchProperties: (location: string) => Promise<void>;
};
...
export const usePropertiesStore = create<propertiesStore>((set) => ({
	...
	loadProperties: async () => {
		...
		set({ properties, /*searchedProperties: properties,*/ loading: false });
	},
	setSearchTerm: (searchTerm: string) => {
		set({ searchTerm });
	},
	// searchProperties: async (location: string) => {
	// 	const state = usePropertiesStore.getState();
	// 	if (!location) {
	// 		set({ searchedProperties: state.properties });
	// 		return;
	// 	}
	// 	set({ loading: true });
	// 	const properties = await PropertiesApi.searchProperties(location);
	// 	set({ searchedProperties: properties, loading: false });
	// },
}));
